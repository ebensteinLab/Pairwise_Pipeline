...
def SSMD_cal(mean_sick, mean_healthy, var_sick, var_healthy, start1, start2, thres_prec):
    ...
    # if start1 == 936000:
    #     indices = np.arange(start1, start1 + (len - start1))
    # else:
    #     indices = np.arange(start1, start1 + 8000)
...
def process_blocks(data_sick, data_healthy, block_size, thres_prec):
    ...
    for i, start1 in enumerate(blocks):
        ...
        # SSMD_res_with_indices, threshold = SSMD_cal(mean_block1_sick, mean_block1_healthy,
        #                                             var_block1_sick, var_block1_healthy, start1, start1)
        #
        # # # feature selection, ignoring last two columns - probe i & probe j idx
        # SSMD_filtered = (SSMD_res_with_indices[:, 0] > np.abs(threshold)) | (
        #         SSMD_res_with_indices[:, 0] < -np.abs(threshold))
        ...
        for j in range(i, np.shape(blocks)[0]):
            ...
            SSMD_res_with_indices, threshold = SSMD_cal(final_pairs_mean_sick, final_pairs_mean_healthy,
                                                        final_pairs_var_sick, final_pairs_var_healthy, start1, start2, thres_prec)
...
def leave_one_out_whole_data(config):
    ...
    print(f"Using thres_prec in run_whole_data_models: {thres_prec}")

    # Concatenate the DataFrames along columns
    df_combined = pd.concat([df_sick, df_healthy], axis=1)
    results = []
    ...
    for col_name, test in df_combined.items():
        print(f"Now Running Test {col_name}")
        
        if "A" in col_name:
            remaining_healthy_for_train = df_healthy.drop(col_name, axis=1)
            remaining_sick_for_train = df_sick
        else:
            remaining_sick_for_train = df_sick.drop(col_name, axis=1)
            remaining_healthy_for_train = df_healthy

        train_set_sick, val_set_sick = split_data(remaining_sick_for_train, fraction=0.8)
        train_set_healthy, val_set_healthy = split_data(remaining_healthy_for_train, fraction=0.8)
        ...
        SSMD_res_with_indices.to_csv(
            os.path.join(config.output_dir, f'SSMD_diff_pairs_{col_name}.csv'), index=False)
        ...
        data_train_for_heatmap.iloc[:, :-1].T.to_csv(
            os.path.join(config.output_dir, f'data_for_heatmap_two_groups_train_only_{col_name}.csv'),
            index=False)
        ...
        #grid_search = GridSearchCV(model, param_grid, scoring='accuracy', cv=kfold, n_jobs=config.n_jobs, verbose=2, error_score=float('nan'))
        ...
        joblib.dump(model, os.path.join(config.output_dir, f'xgb_model_{col_name}.joblib'))

        if "B" in col_name:
            test_sick = df_sick[[col_name]]
            data_after_diff_pairs_sick = test_sick.apply(
                lambda col: cal_pairwise_for_filtered_data(col, SSMD_res_with_indices),
                axis=0)
            data_after_diff_pairs_sick = add_label(data_after_diff_pairs_sick, label=1)
            label = np.array(data_after_diff_pairs_sick['label'].iloc[0]).ravel()
            sick_and_healthy_both_train_and_val_final = data_after_diff_pairs_sick.iloc[:, :-1]
        else:
            test_healthy = df_healthy[[col_name]]
            data_after_diff_pairs_healthy = test_healthy.apply(
                lambda col: cal_pairwise_for_filtered_data(col, SSMD_res_with_indices), axis=0)
            data_after_diff_pairs_healthy = add_label(data_after_diff_pairs_healthy, label=0)
            label = np.array(data_after_diff_pairs_healthy['label'].iloc[0]).ravel()
            sick_and_healthy_both_train_and_val_final = data_after_diff_pairs_healthy.iloc[:, :-1]

        predictions = model.predict(sick_and_healthy_both_train_and_val_final)
        y_scores = model.predict_proba(sick_and_healthy_both_train_and_val_final)[:, 1]

        accuracy = accuracy_score(label, predictions)
        correct_predictions += np.sum(predictions == label)
        incorrect_predictions += np.sum(predictions != label)
        y_true_all.extend(label)
        y_scores_all.extend(y_scores)

        results.append({'model': f'xgb_model_{col_name}', 'accuracy': accuracy})
    
    print("Finished Running All Samples for Models Run")
    results_df = pd.DataFrame(results)
    results_df.to_csv(os.path.join(config.output_dir, "model_prediction_proba_final.csv"), index=False)
